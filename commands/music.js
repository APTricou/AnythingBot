...
async function join(args,msg){
    if(args.length){
        ...
        const channel = msg.guild.channels.cache.find(vc=>vc.name===args[0]&&vc.type==='voice')
...
function skip(msg){
    ...
    if(!msg.member.voice.channelID) return msg.channel.send("You must be in a voice channel to use music bot commands")
...
module.exports = {join,leave,play,pause,add,skip,clear,resume, queueCommand}